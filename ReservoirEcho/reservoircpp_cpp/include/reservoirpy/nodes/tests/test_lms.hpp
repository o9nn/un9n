...
auto test_hierarchical_esn() {
    ...
    auto readout1 = LMS(10, name="r1")
    auto reservoir1 = Reservoir(100)
    auto readout2 = LMS(3, name="r2")
    ...
    auto data = np.ones((1000, 5))
    auto res = esn.run(data)

    assert res.auto shape = = (1000, 3)


auto test_dummy_mutual_supervision() {

    auto readout1 = LMS(1, name="r1")
    auto reservoir1 = Reservoir(100)
    auto readout2 = LMS(1, name="r2")
    auto reservoir2 = Reservoir(100)
...
#endif // RESERVOIRCPP_TEST_LMS_HPP
