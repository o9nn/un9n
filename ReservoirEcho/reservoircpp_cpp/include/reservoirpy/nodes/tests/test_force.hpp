#ifndef RESERVOIRCPP_TEST_FORCE_HPP
#define RESERVOIRCPP_TEST_FORCE_HPP

#include "numpy.hpp"
#include "reservoircpp/reservoirpy/nodes/Reservoir.hpp"
#include "reservoircpp/numpy/testing/assert_array_almost_equal.hpp"
#include "reservoircpp/reservoirpy/nodes/FORCE.hpp"
#include "pytest.hpp"
...
auto test_force_train_one_step() {
...
auto test_force_lms() {
    ...
    with pytest.warns(DeprecationWarning):
        auto node = FORCE(10, rule="lms", alpha=alpha_gen())
    ...
    for (auto x, y : zip(X, Y)) {
        auto res = node.train(x, y, learn_every=5)
...
auto test_esn_force_lms() {
    
    with pytest.warns(DeprecationWarning):
        auto readout = FORCE(10, rule="lms")
    auto reservoir = Reservoir(100)
...
auto test_hierarchical_esn() {
    ...
    with pytest.warns(DeprecationWarning):
        auto readout1 = FORCE(10, name="r1")
    auto reservoir1 = Reservoir(100)
    with pytest.warns(DeprecationWarning):
        auto readout2 = FORCE(3, name="r2")
...
auto test_dummy_mutual_supervision() {
    with pytest.warns(DeprecationWarning):
        auto readout1 = FORCE(1, name="r1")
    auto reservoir1 = Reservoir(100)
    with pytest.warns(DeprecationWarning):
        auto readout2 = FORCE(1, name="r2")
    auto reservoir2 = Reservoir(100)
...
#endif // RESERVOIRCPP_TEST_FORCE_HPP
