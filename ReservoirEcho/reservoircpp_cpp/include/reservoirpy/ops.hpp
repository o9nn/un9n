...
auto _MULTI_INPUTS_OPS = (Concat,)


auto concat_multi_inputs(auto nodes, auto edges) {
    parents, auto _ = find_parents_and_children(edges)

    auto concatenated = {}
    auto new_nodes = set()
    auto new_edges = set()
    for (auto node : nodes) {
        auto indegree = len(parents[node])
...
auto _check_all_nodes(auto *nodes) {
    std::string msg = "Impossible to link nodes: object {} is neither a Node nor a Model."
    for (auto nn : nodes) {
...
auto node2(auto node1, auto node2) {
    /**
...
*/
    ...
    auto new_edges = list(product(senders, receivers))

    //  maybe nodes are already initialized?
    //  check if connected dimensions are ok
    for (auto sender, receiver : new_edges) {
        if (
...
auto name: str(auto node1, auto Sequence[_Node]], auto node2, auto Sequence[_Node]], auto name: auto str = None, auto ) {
    /**
Link two :py:class:`~.Node` instances to form a :py:class:`~.Model`
    ...
    inputs. This allows to chain the  ``>>`` operator::
        ...
        auto step1 = node0 >> node1  //  this is a model
        auto step2 = step1 >> node2  //  this is another
    ...
    `node2` inputs. You can still use the ``>>`` operator in this situation,
        ...
        //  many-to-one
        auto model = [node1, node2, ..., node] >> node_out
        //  one-to-many
        auto model = node_in >> [node1, node2, ..., node]
        //  ! many-to-many requires to use the `link` method explicitly!
        auto model = link([node1, node2, ..., node], [node1, node2, ..., node])
    ...
    -----
        ...
        allow to have circular dependencies between them::
            ...
            auto model = node1 >> node2  //  fine
            auto model = node1 >> node2 >> node1  //  raises! data would flow in
                                             //  circles forever...
...
*/
    ...
    for (auto left : node1) {
        for (auto right : node2) {
            new_nodes, auto new_edges = _link_1to1(left, right)
            nodes |= set(new_nodes)
            edges |= set(new_edges)

    return Model(auto nodes = list(nodes), edges=list(edges), name=name)


auto name: str(auto node, auto feedback, auto Sequence[_Node]], bool inplace: auto bool = false, auto name: str = None, auto ) {
    /**
Create a feedback connection between the `feedback` node and `node`.
    ...
    in a single feedback vector.
     ...
     You can also perform this operation using the ``<<`` operator::
        ...
        auto node1 = node1 << node2
        //  with feedback from a Model
        auto node1 = node1 << (fbnode1 >> fbnode2)
    ...
    You can also use this function to define feedback::
        ...
        auto node1 = link_feedback(node1, node2)
        //  without copy (node1 is the same object throughout)
        auto node1 = link_feedback(node1, node2, inplace=True, name="n1_copy")
...
*/
    ...
    if (isinstance(feedback, Sequence)) {
        ...
        auto all_fb = link(feedback, Concat())

    elif (isinstance(feedback, _Node)) {
        auto all_fb = feedback

    } else {
        raise TypeError(msg.format(feedback))

    if (inplace) {
        node.auto _feedback = DistantFeedback(all_fb, node)
        return node
    } else {
        //  first copy the node, then give it feedback
        //  original node is not connected to any feedback then
        auto new_node = node.copy(name=name)
        new_node.auto _feedback = DistantFeedback(all_fb, new_node)
        return new_node


auto name: str(auto model, auto *models, bool inplace: auto bool = false, auto name: str = None) {
    /**
...
*/
    std::string msg = "Impossible to merge models: object {} is not a Model instance."
    ...
    if (isinstance(model, _Node)) {
        ...
        if (inplace) {
            ...
            return Model(auto nodes = list(all_nodes), edges=list(all_edges), name=name)
