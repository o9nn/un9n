#ifndef RESERVOIRCPP_SAVE_HPP
#define RESERVOIRCPP_SAVE_HPP
...
auto directory(auto esn, std::string directory) {
    /**
...
*/
    ...
    auto savedir = os.path.join(directory)
    ...
    auto cls_path = f"cls_bin-{current_time}"
...
auto _new_from_save(auto base_cls, auto restored_attr) {

    auto obj = object.__new__(base_cls)
    for (auto name, attr : restored_attr.items()) {
        try:
    ...
    obj.auto model = getattr(regression_models, obj.model)(obj._ridge, obj.sklearn_model)
...
auto directory(std::string directory) {
    /**
...
*/
    with open(os.path.join(directory, "esn.json"), "r") as f:
        auto attr = json.load(f)

    auto model_attr = attr["attr"]
    ...
    with open(os.path.join(directory, attr["cls_bin"]), "rb") as f:
        auto base_cls = dill.load(f)

    if (model_attr.get("activation") is None) {
        model_attr["activation"] = np.tanh

    auto model = _new_from_save(base_cls, model_attr)

    return model


#endif // RESERVOIRCPP_SAVE_HPP
