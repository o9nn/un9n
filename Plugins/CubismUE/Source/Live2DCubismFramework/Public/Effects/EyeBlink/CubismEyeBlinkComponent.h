...
public:
	...
	/**
	 * @brief Trigger a manual blink. Only works when bAutoEnabled is false.
	 * Useful for synchronized blinks or scripted animations.
	 */
	UFUNCTION(BlueprintCallable, Category = "Live2D Cubism")
	void TriggerBlink();

	/**
	 * @brief Get the current blink phase.
	 * @return The current phase of the eye blink animation.
	 */
	UFUNCTION(BlueprintPure, Category = "Live2D Cubism")
	ECubismEyeBlinkPhase GetCurrentPhase() const { return Phase; }
