...
set ARM64_LIB_PATH="%PATH_TO_CMAKE_FILE%\..\Lib\WinArm64
...
cd "%VS2015_X64_PATH%"
"%_vsinstall%\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" -G "Visual Studio 17 2022" -A x64 -DCMAKE_SUPPRESS_REGENERATION=1 -DSOCKET_IMPL=../../../src/sock.c -DOPENSSL_PATH="%UE4_OPENSSL_ROOT_DIR%/include/Win64/%UE_TOOLSET_IDE%" -DDISABLE_TLS=0 -DEXPAT_PATH="%UE4_EXPAT_ROOT_DIR%/lib" %PATH_TO_CMAKE_FILE%
...
mkdir "%VS2015_X64_PATH%"
cd "%VS2015_X64_PATH%"
"%_vsinstall%\Common7\IDE\CommonExtensions\Microsoft\CMake\CMake\bin\cmake.exe" -G "Visual Studio 17 2022" -A arm64 -DCMAKE_SUPPRESS_REGENERATION=1 -DSOCKET_IMPL=../../../src/sock.c -DOPENSSL_PATH="%UE4_OPENSSL_ROOT_DIR%/include/Win64/%UE_TOOLSET_IDE%" -DDISABLE_TLS=0 -DEXPAT_PATH="%UE4_EXPAT_ROOT_DIR%/lib" %PATH_TO_CMAKE_FILE%
echo Building libstrophe solution for VS2017 (64-bit arm, Debug)...
"%_msbuild%msbuild.exe" libstrophe.sln /t:build /p:Configuration=Debug
echo Building libstrophe solution for VS2017 (64-bit arm, Release)...
"%_msbuild%msbuild.exe" libstrophe.sln /t:build /p:Configuration=Release
cd "%PATH_TO_CMAKE_FILE%"
mkdir "%ARM64_LIB_PATH%"\Debug
move /y %VS2015_X64_PATH%\Debug\strophe.lib %ARM64_LIB_PATH%\Debug
move /y %VS2015_X64_PATH%\Debug\strophe.pdb %ARM64_LIB_PATH%\Debug
mkdir "%ARM64_LIB_PATH%"\Release
move /y %VS2015_X64_PATH%\Release\strophe.lib %ARM64_LIB_PATH%\Release
echo rmdir "%VS2015_X64_PATH%" /s/q
cd ..
