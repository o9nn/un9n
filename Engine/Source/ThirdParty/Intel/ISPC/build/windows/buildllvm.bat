...
mkdir %LLVM_INSTALL%
cd %LLVM_BUILD%
cmake -Thost=x64 -G "Visual Studio 17" -DCMAKE_INSTALL_PREFIX=%LLVM_INSTALL% -DCMAKE_BUILD_TYPE=Release -DLLVM_ENABLE_PROJECTS="clang" -DLLVM_ENABLE_DUMP=ON -DLLVM_ENABLE_ASSERTIONS=OFF -DLLVM_INSTALL_UTILS=ON -DLLVM_TARGETS_TO_BUILD=AArch64;ARM;X86 -DLLVM_EXPERIMENTAL_TARGETS_TO_BUILD=WebAssembly -DLLVM_LIT_TOOLS_DIR="C:\cygwin64\bin" ..\llvm
